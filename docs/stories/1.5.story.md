...
- [ ] **Task 1: Create API client with auth headers** (AC: 3, 4)

  - [ ] Update `apps/web/lib/api/client.ts` with base fetch wrapper
  - [ ] Add `getAuthToken()` helper to read from localStorage
...
- [ ] **Task 2: Create auth API service** (AC: 1, 2, 3)

  - [ ] Create `apps/web/lib/api/services/auth.ts`
  - [ ] Implement `login(email, password): Promise<LoginResponse>`
  - [ ] Implement `register(data): Promise<User>`
  - [ ] Implement `getMe(): Promise<User>`
  - [ ] Handle API error responses and throw typed errors

- [ ] **Task 3: Create auth context** (AC: 4, 8)

  - [ ] Create `apps/web/lib/contexts/auth-context.tsx`
  - [ ] Provide `user`, `isLoading`, `isAuthenticated` state
  - [ ] Provide `login(email, password)` method
  - [ ] Provide `register(data)` method
  - [ ] Provide `logout()` method - clears token and redirects
  - [ ] On mount, check for existing token and call `getMe()`
  - [ ] Wrap app with `AuthProvider` in `app/layout.tsx`

- [ ] **Task 4: Create protected route component** (AC: 5)

  - [ ] Create `apps/web/lib/components/protected-route.tsx`
  - [ ] Check `isAuthenticated` from auth context
  - [ ] Show loading spinner while checking auth
  - [ ] Redirect to `/login` if not authenticated
  - [ ] Render children if authenticated

- [ ] **Task 5: Wire up LoginForm to auth context** (AC: 1, 3)

  - [ ] Update `apps/web/components/auth/LoginForm.tsx`
  - [ ] Use `login()` from auth context on form submit
  - [ ] Display API error messages (invalid credentials, etc.)
  - [ ] Show loading state during API call
  - [ ] Redirect to home on successful login

- [ ] **Task 6: Wire up RegisterForm to auth context** (AC: 2, 3)

  - [ ] Update `apps/web/components/auth/RegisterForm.tsx`
  - [ ] Use `register()` from auth context on form submit
  - [ ] Add department dropdown with Department enum values
  - [ ] Display API error messages (duplicate email, validation errors)
  - [ ] Show loading state during API call
  - [ ] Auto-login after successful registration OR redirect to login

- [ ] **Task 7: Update login and register pages** (AC: 1, 2)

  - [ ] Update `apps/web/app/login/page.tsx` to use wired LoginForm
  - [ ] Update `apps/web/app/register/page.tsx` to use wired RegisterForm
  - [ ] Add link between login and register pages

- [ ] **Task 8: Wire up UserMenu with logout** (AC: 7, 8)

  - [ ] Update `apps/web/components/layout/UserMenu.tsx`
  - [ ] Display user's `displayName` from auth context
  - [ ] Implement logout button using `logout()` from auth context
  - [ ] Show user's department badge (optional)

- [ ] **Task 9: Protect app routes** (AC: 5, 6)

  - [ ] Wrap home page (`app/page.tsx`) with ProtectedRoute
  - [ ] Wrap folder page (`app/folder/page.tsx`) with ProtectedRoute
  - [ ] Wrap document page (`app/document/page.tsx`) with ProtectedRoute
  - [ ] Wrap editor page (`app/editor/page.tsx`) with ProtectedRoute
  - [ ] Wrap search page (`app/search/page.tsx`) with ProtectedRoute

- [ ] **Task 10: Verify app shell layout** (AC: 6)

  - [ ] Confirm AppShell component has: header, sidebar, main content area
  - [ ] Verify sidebar is present for folder tree (placeholder for Story 1.6)
  - [ ] Verify header includes UserMenu component
