...
- [ ] **Task 1: Create Zod validation schemas for folders and documents** (AC: 2, 5, 6)

  - [ ] Create `packages/shared/src/schemas/folder.ts` with `createFolderSchema`
  - [ ] Create `packages/shared/src/schemas/document.ts` with `createDocumentSchema`, `updateDocumentSchema`
