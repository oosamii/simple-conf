...
- [x] **Task 2: Create folder repository** (AC: 1, 2, 3)

  - [x] Create `apps/api/src/repositories/folder.repository.ts`
  - [x] Implement `findAll(): Promise<Folder[]>` - get all folders
  - [x] Implement `findById(id: string): Promise<Folder | null>`
  - [x] Implement `findChildren(parentId: string | null): Promise<Folder[]>`
  - [x] Implement `create(data: CreateFolderData): Promise<Folder>`
  - [x] Implement `getDocumentCount(folderId: string): Promise<number>`

- [x] **Task 3: Create document repository** (AC: 4, 5, 6, 7, 9)

  - [x] Create `apps/api/src/repositories/document.repository.ts`
  - [x] Implement `findById(id: string): Promise<Document | null>`
  - [x] Implement `findByFolderId(folderId: string): Promise<DocumentSummary[]>`
  - [x] Implement `create(data: CreateDocumentData): Promise<Document>`
  - [x] Implement `update(id: string, data: UpdateDocumentData): Promise<Document>`
  - [x] Implement `delete(id: string): Promise<void>`
  - [x] Implement `findWithMeta(id: string): Promise<DocumentWithMeta | null>` - includes user info

- [x] **Task 4: Create folder service** (AC: 1, 2, 3)

  - [x] Create `apps/api/src/services/folder.service.ts`
  - [x] Implement `getFolderTree(userId: string): Promise<FolderTreeNode[]>` - build tree with document counts
  - [x] Implement `getFolderDetails(id: string, userId: string): Promise<FolderDetailResponse>`
  - [x] Implement `createFolder(data, userId): Promise<Folder>`
  - [x] Handle folder visibility/access based on user department

- [x] **Task 5: Create document service** (AC: 4, 5, 6, 7, 9)

  - [x] Create `apps/api/src/services/document.service.ts`
  - [x] Implement `getDocument(id: string, userId: string): Promise<DocumentWithMeta>`
  - [x] Implement `createDocument(data, userId): Promise<Document>`
  - [x] Implement `updateDocument(id, data, userId): Promise<Document>` - verify ownership
  - [x] Implement `deleteDocument(id, userId): Promise<void>` - verify ownership

- [x] **Task 6: Create folder routes module** (AC: 1, 2, 3, 8)

  - [x] Create `apps/api/src/modules/folders/folders.routes.ts`
  - [x] `GET /folders` - require auth, return folder tree
  - [x] `POST /folders` - require auth, validate input, create folder
  - [x] `GET /folders/:id` - require auth, return folder with contents
  - [x] Create `apps/api/src/modules/folders/index.ts`
  - [x] Register folder routes in `apps/api/src/app.ts`

- [x] **Task 7: Create document routes module** (AC: 4, 5, 6, 7, 8, 9)

  - [x] Create `apps/api/src/modules/documents/documents.routes.ts`
  - [x] `GET /documents/:id` - require auth, return full document with meta
  - [x] `POST /documents` - require auth, validate input, create document
  - [x] `PUT /documents/:id` - require auth, validate input, verify owner, update
  - [x] `DELETE /documents/:id` - require auth, verify owner, delete
  - [x] Create `apps/api/src/modules/documents/index.ts`
  - [x] Register document routes in `apps/api/src/app.ts`

- [x] **Task 8: Add API response types** (AC: 9)

  - [x] Update `packages/shared/src/types/api.ts` with folder/document response types
  - [x] Add `FolderTreeResponse`, `FolderDetailResponse`
  - [x] Add `DocumentResponse`, `CreateDocumentResponse`
